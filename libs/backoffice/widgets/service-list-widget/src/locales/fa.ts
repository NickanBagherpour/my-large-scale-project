export default {
  widget_name: 'لیست سرویس‌ها',
  upload_service: 'بارگذاری سرویس',
  create_new_service: 'ایجاد سرویس جدید',
  all_services: 'همه سرویس‌ها',
  operational_services: 'سرویس‌های عملیاتی',
  stopped_services: 'سرویس‌های متوقف شده',
  draft: 'پیش‌نویس‌',
  // get_info: 'دریافت اطلاعات',
  // add_service: 'افزودن سرویس',
  // add_plugin: 'افزودن پلاگین',
  service_name: 'نام سرویس',
  service_info: 'اطلاعات سرویس',
  add_documents: 'افزودن مستندات',
  operationalization: 'عملیاتی‌سازی',
  row: 'ردیف',
  name: 'نام',
  persian_name: 'نام فارسی',
  scope: 'اسکوپ',
  version: 'نسخه',
  status: 'وضعیت',
  url: 'آدرس',
  operational: 'عملیاتی',
  stopped: 'توقف',
  stop: 'متوقف',
  detailed: 'جزئیات',
  delete_service: 'حذف سرویس',
  service_question: 'آیا سرویس',
  done_stopped: 'شود؟',
  add_service_from_sso: 'افزودن سرویس از SSO',
  services_english_name: 'نام انگلیسی سرویس',
  oauth_key: 'OAuth Key',
  next_level: 'مرحله بعد',
  inspect_another: 'استعلام سرویس دیگر',
  allowed_creation: 'مجاز به ثبت سرویس می‌باشید. سرویس با این مشخصات در سامانه ثبت نشده است.',
  allowed_creation_BAAM:
    'مجاز به بارگذاری سرویس می‌باشید. سرویس مورد نظر در سامانه بام ثبت شده است. لطفاً جهت بارگذاری سرویس در اکسیژن از طریق دکمه زیر اقدام نمایید.',
  draft_delete_title: 'حذف پیش نویس',
  draft_delete_confirm: 'آیا از حذف پیش نویس',
  complete_service_info:
    'مجاز به ثبت سرویس می‌باشید. سرویس مورد نظر تکمیل نشده است. لطفا نسبت به تکمیل فرایند ثبت سرویس اقدام نمایید. ',
  already_exists:
    'مجاز به ثبت سرویس با این مشخصات نمی‌باشید. سرویس مورد نظر قبلاً با مشخصات زیر در سامانه ثبت شده است. ',
  placeholders: {
    search_service_inspection: 'جستجو بر‌اساس نام انگلیسی سرویس',
  },
  buttons: {
    inspect: 'استعلام سرویس',
    complete_service_info: 'تکمیل اطلاعات سرویس',
  },
  general_info: 'اطلاعات کلی',
  route: 'Route',
  upstream: 'آپ استریم',
  info_confirm: 'تایید اطلاعات',
  is_connected: ' متصل است.',
  with_activation: '  با عملیاتی‌سازی این سرویس، این ',
  activation_question: '  قادر به استفاده از آن خواهند بود. آیا از عملیاتی‌سازی آن اطمینان دارید؟',
  deactivation_question: ' آیا از توقف آن اطمینان دارید؟',
  activation_confirm: 'آیا از عملیاتی‌سازی سرویس ',
  are_u_sure: ' اطمینان دارید؟',
  deactivation_confirm: ' آیا از توقف سرویس ',
  service_activation: 'عملیاتی‌سازی سرویس',
  stop_service: 'توقف سرویس',
  to: ' به ',
};
