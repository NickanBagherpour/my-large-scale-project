...
export const createServerType = (t: TFunction) => {
  return z.object({
    // [FORM_ITEM_NAMES.domain]: regexString(REGEX_PATTERNS.isLatinText, 'error.english_validation_message', t).max(
    //   150,
    //   t('validation.max_len', { val: 150 })
    // ),
    // [FORM_ITEM_NAMES.weight]: regexString(REGEX_PATTERNS.isPersianText, 'error.persian_validation_message', t).max(
    //   150,
    //   t('validation.max_len', { val: 150 })
    // ),
  });
